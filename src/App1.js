...
export default function App() {
  ...
  // ── All state starts empty — add your own items via the app ───────────────
...
function HomePage({ lowStock, inventory, setTab }) {
  ...
  // Alphabetical for the overview list
  ...
  return (
    <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {inventory.length > 0 ? (
        ...
        <Section title="📋 Inventory Overview">
          <Empty text="No items yet — add stock to get started" />
          <button onClick={() => setTab("addStock")} style={{ ...btnStyle(C.primary), marginTop: 8 }}>Add First Item</button>
        </Section>
      )}
    </div>
  );
...
function InventoryTab({ inventory }) {
  ...
  const filtered = useMemo(() => {
    ...
    else list = [...list].sort((a, b) => a.name.localeCompare(b.name)); // default: alphabetical
    return list;
...
function InventoryCard({ item }) {
  ...
  return (
    <div style={{ borderRadius: 12, padding: 14, border: `1px solid ${isLow ? "#FCA5A5" : C.border}`, background: isLow ? C.lowBg : "#fff" }}>
...
function AddStockTab({ inventory, setInventory, setPurchases, setMovements }) {
  ...
  // Alphabetical search suggestions
            ...
            <div style={{ position: "relative" }}>
              ...
              {matches.length > 0 && (
                <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", border: `1px solid ${C.border}`, borderRadius: 8, zIndex: 10, boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}>
                  {matches.map((m) => (
                    <div key={m.id} onClick={() => selectExisting(m)} style={{ padding: "10px 14px", cursor: "pointer", fontSize: 13, borderBottom: `1px solid ${C.border}` }}>
                      {m.name} <span style={{ color: C.muted }}>({m.qty} {m.unit})</span>
                    </div>
                  ))}
                </div>
              )}
...
function PurchaseHistoryTab({ purchases }) {
  ...
  const list = useMemo(() => {
    ...
    else l = [...l].sort((a, b) => a.item.localeCompare(b.item)); // alphabetical by item
    return l;
  ...
  return (
    <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {list.length === 0
        ? <Empty text="No purchase records yet" />
        : list.map((p) => (
